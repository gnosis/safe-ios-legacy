// swiftlint:disable file_header colon line_length todo operator_usage_whitespace
// swiftlint:disable trailing_whitespace trailing_closure implicit_return unneeded_parentheses_in_closure_argument
// swiftlint:disable extension_access_modifier return_arrow_whitespace number_separator redundant_discardable_let multiline_parameters pattern_matching_keywords control_statement
...
/// `OBJCJSONEncoder` facilitates the encoding of `Encodable` values into JSON in form of objc objects.
open class OBJCJSONEncoder {
    // MARK: Options
    ...
    /// - returns: A new root object of tree of objects representing JSON.
    /// - throws: `EncodingError.invalidValue` if a non-conforming floating-point value is encountered during encoding, and the encoding strategy is `.throw`.
    /// - throws: An error if any value throws an error during encoding.
    open func encode<T : Encodable>(_ value: T) throws -> AnyObject {
        let encoder = _OBJCJSONEncoder(options: self.options)
...
// MARK: - _OBJCJSONEncoder

fileprivate class _OBJCJSONEncoder : Encoder {
    // MARK: Properties
    ...
    fileprivate let options: OBJCJSONEncoder._Options
    ...
    fileprivate init(options: OBJCJSONEncoder._Options, codingPath: [CodingKey] = []) {
...
fileprivate struct _JSONKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    private let encoder: _OBJCJSONEncoder
    ...
    fileprivate init(referencing encoder: _OBJCJSONEncoder, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = OBJCJSONEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
...
fileprivate struct _JSONUnkeyedEncodingContainer : UnkeyedEncodingContainer {
    ...
    /// Initializes `self` with the given references.
    fileprivate init(referencing encoder: _OBJCJSONEncoder, codingPath: [CodingKey], wrapping container: NSMutableArray) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
...
extension _OBJCJSONEncoder : SingleValueEncodingContainer {
...
extension _OBJCJSONEncoder {
    ...
    fileprivate func box_(_ value: Encodable) throws -> NSObject? {
        ...
        // The value should request a container from the _OBJCJSONEncoder.
        let depth = self.storage.count
        do {
...
/// _JSONReferencingEncoder is a special subclass of _OBJCJSONEncoder which has its own storage, but references the contents of a different encoder.
/// It's used in superEncoder(), which returns a new encoder for encoding a superclass -- the lifetime of the encoder should not escape the scope it's created in, but it doesn't necessarily know when it's done being used (to write to the original container).
fileprivate class _JSONReferencingEncoder : _OBJCJSONEncoder {
    ...
    fileprivate let encoder: _OBJCJSONEncoder
    ...
    fileprivate init(referencing encoder: _OBJCJSONEncoder, at index: Int, wrapping array: NSMutableArray) {
        self.encoder = encoder
    ...
    fileprivate init(referencing encoder: _OBJCJSONEncoder,
...
/// `OBJCJSONDecoder` facilitates the decoding of tree of primitive OBJC types representing JSON into semantic `Decodable` types.
open class OBJCJSONDecoder {
    // MARK: Options
    ...
    /// - parameter topLevel: top level JSON object to decode from.
    /// - returns: A value of the requested type.
    /// - throws: `DecodingError.dataCorrupted` if values requested from the payload are corrupted, or if the given data is not valid JSON.
    /// - throws: An error if any value throws an error during decoding.
    open func decode<T : Decodable>(_ type: T.Type, from topLevel: Any) throws -> T {
        let decoder = _OBJCJSONDecoder(referencing: topLevel, options: self.options)
...
// MARK: - _OBJCJSONDecoder

fileprivate class _OBJCJSONDecoder : Decoder {
    // MARK: Properties
    ...
    fileprivate let options: OBJCJSONDecoder._Options
    ...
    fileprivate init(referencing container: Any, at codingPath: [CodingKey] = [], options: OBJCJSONDecoder._Options) {
...
fileprivate struct _JSONKeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol {
    ...
    private let decoder: _OBJCJSONDecoder
    ...
    fileprivate init(referencing decoder: _OBJCJSONDecoder, wrapping container: [String : Any]) {
        ...
        case .convertFromSnakeCase:
            ...
            self.container = Dictionary(container.map {
                key, value in (OBJCJSONDecoder.KeyDecodingStrategy._convertFromSnakeCase(key), value)
    ...
    private func _errorDescription(of key: CodingKey) -> String {
        ...
        case .convertFromSnakeCase:
            ...
            let converted = OBJCJSONEncoder.KeyEncodingStrategy._convertToSnakeCase(original)
    ...
    private func _superDecoder(forKey key: __owned CodingKey) throws -> Decoder {
        ...
        return _OBJCJSONDecoder(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
    }
...
fileprivate struct _JSONUnkeyedDecodingContainer : UnkeyedDecodingContainer {
    ...
    /// Initializes `self` by referencing the given decoder and container.
    fileprivate init(referencing decoder: _OBJCJSONDecoder, wrapping container: [Any]) {
        self.decoder = decoder
        self.container = container
        self.codingPath = decoder.codingPath
        self.currentIndex = 0
...
extension _OBJCJSONDecoder : SingleValueDecodingContainer {
