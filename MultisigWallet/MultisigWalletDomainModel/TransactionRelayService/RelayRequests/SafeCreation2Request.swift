...
/// of a creation transaction partly on the client side, partly on the app side,
/// and based on that signature's owner nonce we would generate contract address.
public struct SafeCreation2Request: Encodable {

    /// Number used once, generated by the client (the app). **Important**: use random value.
    public let saltNonce: Int // Isn't it 256-bit?
    ...
    public let paymentToken: String // Optional!

    public init(saltNonce: Int, owners: [Address], confirmationCount: Int, paymentToken: Address) {
